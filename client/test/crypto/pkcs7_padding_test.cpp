...
BOOST_AUTO_TEST_CASE(add_padding_test)
{
    ...
    chat::crypto::pkcs7::add_padding(input, 16, 10);
...
BOOST_AUTO_TEST_CASE(add_padding_for_empty_block_test)
{
    ...
    chat::crypto::pkcs7::add_padding(input, 16, 0);

    BOOST_CHECK_EQUAL_COLLECTIONS(input, input + 16, expected, expected + 16);
}

BOOST_AUTO_TEST_CASE(add_padding_for_full_data_block_test)
{
    ...
    chat::crypto::pkcs7::add_padding(input, 16, 16);

    BOOST_CHECK_EQUAL_COLLECTIONS(input, input + 32, expected, expected + 32);
}
...
BOOST_AUTO_TEST_CASE(add_padding_for_invalid_data_size)
{
    ...
    BOOST_CHECK_THROW(chat::crypto::pkcs7::add_padding(input, 16, 20), std::runtime_error);
}
...
BOOST_AUTO_TEST_CASE(remove_padding_test)
{
    ...
    BOOST_CHECK_EQUAL(chat::crypto::pkcs7::remove_padding(input, 16), 4);
}

BOOST_AUTO_TEST_CASE(remove_padding_from_whole_data_block_test)
{
    unsigned char input[16] =
    {
        0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10,
    ...
    BOOST_CHECK_EQUAL(chat::crypto::pkcs7::remove_padding(input, 16), 16);
}
...
BOOST_AUTO_TEST_CASE(remove_padding_from_invalid_data_test)
{
    ...
    BOOST_CHECK_THROW(chat::crypto::pkcs7::remove_padding(input, 16), std::runtime_error);
}
